...
patch('spaceship-ui/src/content.config.ts',"    lang: z.literal('en'), category:","    publicationTimeZone: z.string().default('Asia/Seoul'),\n    lang: z.literal('en'), category:")
...
# Assert the displayed edition day independently of the machine's timezone.
patch('spaceship-ui/scripts/sep25-contract.py',"p=Page(text);assert any(t=='html'", "p=Page(text);assert re.search(r'<time[^>]*>\\s*2026-09-25\\s*</time>',text)\n assert any(t=='html'")
j['files']=[]
