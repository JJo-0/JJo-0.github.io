...
const catalogue=JSON.parse(fs.readFileSync(new URL('../site/news-edition-20260921.json',import.meta.url),'utf8'));
let edition,route;
...
try{
  ...
  for (edition of catalogue.entries) for(const width of [390,1440])for(const dark of [false,true]){
    ...
    assert(await js(`Array.from(document.querySelectorAll('time')).some(t=>t.dateTime==='2026-09-20T15:00:00.000Z'&&t.textContent.includes('Sep 21, 2026'))`),'Correct publication instant and Korean date');
    ...
    for(const name of edition.equations){
      ...
      if(name===edition.equations[0]||name==='fiber-prior'||name==='erled-rates')await shot(`${name}-equation-${width}-${dark?'dark':'light'}`);
  ...
  process.exitCode=1;console.error('sep21-browser: FAIL',error);
