...
const catalogue=JSON.parse(fs.readFileSync(new URL('../site/news-wetlab-20260919.json',import.meta.url),'utf8'));
let edition,route;
const media=JSON.parse(fs.readFileSync(new URL('../site/news-media.json',import.meta.url),'utf8'));

const out='wetlab-review';fs.mkdirSync(out,{recursive:true});
const results=[];let cdp,sessionId,preview,chrome,activeCase,lastAction;
const deadline=setTimeout(()=>{console.error('wetlab-browser: FAIL hard timeout');process.exit(1);},240_000);
...
try{
  ...
  for (edition of catalogue.entries) for(const width of [390,1440])for(const dark of [false,true]){
    ...
    for(const tableId of ['wetlab-roles','wetlab-results','wetlab-timeline','wetlab-next']){
    ...
    console.log('wetlab-browser: PASS '+JSON.stringify(results.at(-1)));
  }
  ...
  process.exitCode=1;console.error('wetlab-browser: FAIL',error);
