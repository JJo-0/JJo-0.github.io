...
async function auditResearchNodeClicks(cdp, sessionId) {
  ...
  console.log(
    'browser-smoke: PASS Research ASCII focus synchronization and four-node click routing'
  );
  return ids;
}
...
export async function auditRenderer(cdp, sessionId) {
  ...
  const homeAscii = await optionalExpression(
    cdp,
    sessionId,
    `document.querySelector('[data-experience-canvas="home"]')?.dataset.asciiGraph === 'true' &&
     document.querySelector('[data-experience-canvas="home"]')?.dataset.rendererStatus === 'fallback'`
  );
  assert.equal(homeAscii, true, 'Home ASCII writing graph unavailable');
  ...
  const researchAscii = await optionalExpression(
    cdp,
    sessionId,
    `document.querySelector('[data-experience-canvas="research"]')?.dataset.asciiGraph === 'true'`
  );
  assert.equal(researchAscii, true, 'Research ASCII writing graph unavailable');
  console.log('browser-smoke: PASS Research state/ASCII matrix');

  const safeCases = [
    {
      options: { width: 680, height: 820 },
      label: 'narrow fine-pointer SAFE parity',
      expression: `document.querySelector('[data-experience-canvas="home"]')?.dataset.rendererTier === 'safe'`,
    },
    {
