...
const catalogue=JSON.parse(fs.readFileSync(new URL('../site/news-edition-20260920.json',import.meta.url),'utf8'));
let edition,route;
const media=JSON.parse(fs.readFileSync(new URL('../site/news-media.json',import.meta.url),'utf8'));

const out='sep20-review';fs.mkdirSync(out,{recursive:true});
const results=[];let cdp,sessionId,preview,chrome,activeCase,lastAction;
const deadline=setTimeout(()=>{console.error('sep20-browser: FAIL hard timeout');process.exit(1);},240_000);
...
try{
  ...
  for (edition of catalogue.entries) for(const width of [390,1440])for(const dark of [false,true]){
    ...
    assert(await js(`Array.from(document.querySelectorAll('time')).some(t=>t.dateTime==='2026-09-19T15:00:00.000Z'&&t.textContent.includes('Sep 20, 2026'))`),'Correct publication instant and Korean date');
    ...
    console.log('sep20-browser: PASS '+JSON.stringify(results.at(-1)));
  }
  ...
  process.exitCode=1;console.error('sep20-browser: FAIL',error);
