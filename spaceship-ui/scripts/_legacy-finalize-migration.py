...
# 1. Migrate the only live legacy assets to owned namespaces.
moves = {
...
# Current-orphan, obsolete, or byte-duplicate files. Git history remains the archive.
for name in [
    'AMR_.png',
    'AMR_Sample_V1.mp4',
    'AMR_Sample_V2.mp4',
...
# 2. Normalize the two still-live fragment aliases at source.
...
# 3. Convert only the six audited CommonMark indented HTML-code blocks back to source HTML.
html_posts = [
    '2025-05-20.md',
    '2025-06-27.md',
    '2025-07-23-2.md',
...
def dedent_legacy_html(path: Path) -> int:
...
for name in html_posts:
    print(f'dedented {name}: {dedent_legacy_html(posts / name)} HTML block(s)')

# 4. Normalize only emphasis known to survive CommonMark as literal ** markers.
strong_posts = [
    'raspberry-pi-5.md',
...
for name in strong_posts:
    ...
    if replacements[0]:
        print(f'normalized literal strong {name}: {replacements[0]}')
        path.write_text(new_text, encoding='utf-8')

# 5. Remove all three compatibility shims from Astro and delete their implementations.
