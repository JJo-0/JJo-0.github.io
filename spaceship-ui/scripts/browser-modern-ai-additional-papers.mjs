...
const manifest = JSON.parse(fs.readFileSync('site/assets/assets/posts/modern-ai-additional-papers/figures.json', 'utf8'));
const results = [];
const out = 'continuation-review/additional';
fs.mkdirSync(out, { recursive: true });
let preview, chrome, cdp, sessionId, targetId, active;
const deadline = setTimeout(() => { console.error('additional-papers-browser: FAIL hard deadline'); process.exit(1); }, 180_000);
async function shot(name) {
  const {data} = await cdp.send('Page.captureScreenshot', {format:'png'}, sessionId);
  fs.writeFileSync(`${out}/${name}.png`, Buffer.from(data,'base64'));
}
async function originalPopup(row, mobile) {
  const selector = `[data-additional-figure="${row.id}"] .additional-image`;
  ...
  for(const width of [390,1440]) {
    ...
    for(const dark of [false,true]) {
      for(const part of [7,8]) {
        ...
        const ids=await evaluate(cdp,sessionId,`[...document.querySelectorAll('article [data-additional-figure]')].map(f=>f.dataset.additionalFigure)`);
        assert.deepEqual(ids,rows.map(r=>r.id));
        for(const row of rows) {
          active.figure=row.id;
          await evaluate(cdp,sessionId,`document.querySelector('[data-additional-figure="${row.id}"]').scrollIntoView({block:'center',behavior:'instant'})`);
          await waitExpression(cdp,sessionId,`(() => {const i=document.querySelector('[data-additional-figure="${row.id}"] img');return i?.complete&&i.naturalWidth===${row.width}&&i.naturalHeight===${row.height};})()`, `decoded ${row.id}`);
          const info=await evaluate(cdp,sessionId,`(() => {
            const f=document.querySelector('[data-additional-figure="${row.id}"]'),i=f.querySelector('img'),r=i.getBoundingClientRect();
            return {width:r.width,height:r.height,alt:i.alt,filter:getComputedStyle(i).filter,src:i.getAttribute('src'),caption:f.querySelector('figcaption').textContent,links:[...f.querySelectorAll('.additional-credit a')].map(a=>a.href),overflow:document.documentElement.scrollWidth>innerWidth+2};
          })()`);
          ...
          assert.equal(info.src,`/assets/posts/modern-ai-additional-papers/${row.file}`);
          await originalPopup(row,width===390);
          await evaluate(cdp,sessionId,`(() => {const f=document.querySelector('[data-additional-figure="${row.id}"]');f.scrollIntoView({block:'start',behavior:'instant'});scrollBy(0,-145);})()`);
        ...
        assert.equal(end.oldFigures,0);
        const priorFigures=await evaluate(cdp,sessionId,`[...document.querySelectorAll('article [data-reading-figure]')].map(f=>f.dataset.readingFigure)`);
        assert.deepEqual(priorFigures,part===7?['vae-fig-1','ddpm-fig-2']:['simclr-fig-2','clip-fig-1']);
        results.push({...active,figures:rows.map(r=>r.id),nativeFullSize:true,equations:end.equations});
        console.log('additional-papers-browser: PASS '+JSON.stringify(results.at(-1)));
      }
    }
  }
  assert.equal(results.length,8);
  fs.writeFileSync(`${out}/browser.json`,JSON.stringify({base:BASE,results,nativeOriginalActivations:40},null,2));
  console.log('additional-papers-browser: PASS 8 article/viewport/theme cases; 40 trusted original-image activations');
} catch(error) {
  console.error('additional-papers-browser: FAIL',error);process.exitCode=1;
