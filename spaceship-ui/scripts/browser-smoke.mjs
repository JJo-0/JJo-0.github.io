...
async function startChrome() {
  ...
  const child = spawn(chrome, [
    ...
    '--disable-extensions', '--disable-sync', '--mute-audio', '--no-first-run', '--enable-webgl',
...
async function viewport(cdp, sessionId, {
  width, height, mobile = false, touch = false, reduced = false,
}) {
...
async function navigate(cdp, sessionId, pathname) {
  ...
  await poll(
    () => evaluate(cdp, sessionId, `location.href.startsWith(${JSON.stringify(url)}) && document.readyState === 'complete'`),
    `navigation ${pathname}`,
  );
  await sleep(300);
}
...
async function main() {
  ...
  try {
    ...
    const home = await waitExpression(cdp, sessionId, `(() => {
      ...
      const b = brand.getBoundingClientRect(); const s = stage.getBoundingClientRect();
      return {
        ...
        inside: overlays.every((node) => { const r=node.getBoundingClientRect(); return r.left>=s.left-1&&r.right<=s.right+1&&r.top>=s.top-1&&r.bottom<=s.bottom+1; }),
      };
    ...
    await evaluate(cdp, sessionId, `(() => {
      ...
      document.querySelector('[data-constellation-node]')?.focus(); return true;
    })()`);
    const homeId = await waitExpression(cdp, sessionId,
      `window.__jjoSmokeState?.activeResearchNode || null`,
      'Home card to shared state synchronization');
    assert.equal(await evaluate(cdp, sessionId,
      `document.querySelector('[data-constellation-node][data-active]')?.dataset.constellationNode || null`),
      homeId);
    ...
    if (homeGpu) {
      await viewport(cdp, sessionId, { width: 680, height: 820 });
      await waitExpression(cdp, sessionId, `(() => { const h=document.querySelector('[data-experience-canvas="home"]'); return h?.dataset.rendererTier==='safe'&&!h.dataset.rendererCore; })()`, 'live desktop-to-SAFE reclassification');
      await viewport(cdp, sessionId, { width: 1440, height: 900 });
      await waitExpression(cdp, sessionId, `document.querySelector('[data-experience-canvas="home"]')?.dataset.rendererStatus === 'active'`, 'live SAFE recovery');
    }
    ...
    await evaluate(cdp, sessionId, `(() => {
      window.__jjoSmokeState=null;
      addEventListener('jjo:experience-state',(event)=>{window.__jjoSmokeState=event.detail});
      document.querySelectorAll('[data-constellation-node]')[1]?.focus(); return true;
    })()`);
    await waitExpression(cdp, sessionId,
      `window.__jjoSmokeState?.activeResearchNode === ${JSON.stringify(ids[1])}`,
    ...
    const researchGpu = await optionalExpression(cdp, sessionId,
      `document.querySelector('[data-experience-canvas="research"]')?.dataset.rendererStatus === 'active'`);
    if (researchGpu) {
      await waitExpression(cdp, sessionId,
    ...
    await navigate(cdp, sessionId, '/');
    await waitExpression(cdp, sessionId,
      `(() => { const h=document.querySelector('[data-experience-canvas="home"]'); return h?.dataset.rendererTier==='safe'&&!h.dataset.rendererCore; })()`,
    ...
    await navigate(cdp, sessionId, '/');
    const mobile = await waitExpression(cdp, sessionId, `(() => {
      const brand=document.querySelector('[data-site-brand]'); const h=document.querySelector('[data-experience-canvas="home"]');
      if(!brand||!h||!h.dataset.rendererTier)return null; const b=brand.getBoundingClientRect();
      return {left:b.left,clipped:brand.scrollWidth>brand.clientWidth+1,overflow:document.documentElement.scrollWidth>innerWidth+1,coarse:matchMedia('(pointer:coarse)').matches,tier:h.dataset.rendererTier,core:h.dataset.rendererCore||null,display:getComputedStyle(h).display};
    })()`, 'mobile SAFE/header metrics');
    header(mobile, 'mobile');
    assert.equal(mobile.coarse, true); assert.equal(mobile.tier, 'safe');
    assert.equal(mobile.core, null); assert.equal(mobile.display, 'none');
    ...
    await navigate(cdp, sessionId, '/');
    await waitExpression(cdp, sessionId,
      `(() => { const h=document.querySelector('[data-experience-canvas="home"]'); return matchMedia('(pointer:coarse)').matches&&h?.dataset.rendererTier==='safe'&&!h.dataset.rendererCore; })()`,
    ...
    await navigate(cdp, sessionId, '/');
    await waitExpression(cdp, sessionId,
      `(() => { const h=document.querySelector('[data-experience-canvas="home"]'); const r=document.querySelector('[data-experience-page]'); return matchMedia('(prefers-reduced-motion:reduce)').matches&&h?.dataset.rendererTier==='safe'&&!h.dataset.rendererCore&&r?.dataset.motionMode==='reduced'; })()`,
    ...
    const article = await waitExpression(cdp, sessionId, `(() => [...document.querySelectorAll('a[href^="/posts/"]')].find((a)=>!a.getAttribute('href').startsWith('/posts/tag/'))?.getAttribute('href')||null)()`, 'article URL');
    await navigate(cdp, sessionId, article);
    assert.deepEqual(await evaluate(cdp, sessionId, `({canvas:document.querySelectorAll('[data-experience-canvas]').length,runtime:Boolean(window.__jjoRendererRuntime),core:document.querySelectorAll('[data-renderer-core]').length})`),
    ...
    if (chrome?.profile) {
      try { fs.rmSync(chrome.profile, { recursive: true, force: true, maxRetries: 3 }); } catch {}
    }
  }
}
