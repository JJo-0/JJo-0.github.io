...
const anchorInventory = `(() => {
  const normalize = ${normalizeSource};
  const isVisible = ${visibleSource};
  const regionOf = ${regionSource};
  ...
  return visibleAnchors.map((anchor) => {
    ...
    const sameIdentity = visibleAnchors.filter((candidate) =>
    ...
    return {
      ...
      aria,
      region,
      ordinal: sameIdentity.indexOf(anchor),
...
async function clickRouteLinks(cdp, sessionId, route) {
  ...
  for (const [index, anchor] of anchors.entries()) {
    ...
    await waitExpression(
      cdp,
      sessionId,
      targetExpression(anchor, `return Boolean(target);`),
...
async function auditSearch(cdp, sessionId) {
  ...
  await evaluate(cdp, sessionId, `document.querySelector(${JSON.stringify(`a[href="${href}"]`)})?.click()`);
