...
## 핵심 요약
- Gemini Deep Research는 자료 수집보다 연구 질문 설계와 프롬프트 구조화에서 성능 차이가 크게 납니다.
- 이 글은 실무형 프롬프트 템플릿과 실패 패턴 교정 방식을 함께 제공합니다.
- 각 섹션을 순서대로 따라가면 조사 범위 정의부터 결과 검증까지 한 흐름으로 적용할 수 있습니다.

## 빠른 탐색
1. 리서치 목표 정의와 범위 제한
2. 결과 품질을 높이는 프롬프트 패턴
3. 재현 가능한 검증/요약 워크플로우
