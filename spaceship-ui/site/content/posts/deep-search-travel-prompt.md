...
## 핵심 요약
- 여행 리서치 프롬프트는 일정 생성보다 제약 조건(예산·이동·취향) 모델링이 결과 품질을 좌우합니다.
- 이 글은 Deep Research를 여행 계획에 맞게 튜닝하는 프롬프트 설계법을 단계별로 제공합니다.
- 체크리스트 기반 구조로 초안 생성부터 검증/수정까지 반복 가능한 흐름을 만들 수 있습니다.

## 빠른 탐색
1. 여행 시나리오별 프롬프트 템플릿
2. 결과 오류를 줄이는 검증 질문 세트
3. 일정·비용·동선 통합 정리 방법
