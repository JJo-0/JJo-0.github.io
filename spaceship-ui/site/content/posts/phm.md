...
이 글은 2024년 연구실에서 다룬 모터 진동 데이터와 당시 Python 코드를 2026년 기준으로 다시 감사한 기록이다. 새로 제공된 발표자료와 「고장 진단에 대한 연구」 PDF를 함께 검토해 실험 배경과 판단 기준을 보강했다. 다만 원 데이터는 비공개이므로 수치와 그림을 공개 벤치마크 성능처럼 일반화하지 않는다.
...
### 관련 PDF와 이 데이터는 같은 실험인가

같다고 단정하면 안 된다. 「고장 진단에 대한 연구」는 산업용 로봇의 하모닉 드라이브를 대상으로 정상, 플렉스 스플라인 결함, 백래시 조건을 다루고 진동 센서와 3축 가속도 센서를 비교한다. 반면 이 글의 Python 파일과 발표자료는 `H`, `B`, `F` 같은 센서 그룹, 속도·하중을 파일명에서 읽는 별도 전처리 실습이다.

PDF는 **좋은 실험 설계의 배경 자료**, 발표자료와 코드는 **당시 파이프라인의 직접 증거**로 사용했다. 두 자료의 라벨을 억지로 합치지 않았다.
...
- 내부 자료: 박지호, 「고장 진단에 대한 연구」, 69쪽 연구 문서. 하모닉 드라이브 고장 유형, 센서 수집, 전처리와 평가 설계를 교차 확인하는 데 사용했다.
