...
description: '지속적인 보통 수준의 카페인 섭취가 신체적, 정신적, 신경학적 영향에 대한 심층 분석'
pubDate: 2025-05-20
tags: ['health-wellness', 'nutrition', 'neuroscience', 'research-analysis', 'areas']
lang: 'ko'
---

## 핵심 요약
- 카페인은 적정량에서는 집중력·각성 개선에 도움을 줄 수 있지만, 만성 섭취 시 수면·불안·내성 문제를 함께 관리해야 합니다.
- 이 글은 신체·정신·신경학적 영향 데이터를 한 번에 비교할 수 있도록 구성했습니다.
- 인터랙티브 그래프와 카드형 요약으로 리스크와 이점을 균형 있게 확인할 수 있습니다.

## 이 글에서 다루는 질문
1. 지속적 카페인 섭취가 인지·기분·수면에 주는 영향은 무엇인가
2. 개인별 민감도와 내성 형성은 어떻게 다른가
3. 실천 가능한 섭취 가이드라인은 무엇인가
...
<style>
    .interactive-report-body {
        ...
        letter-spacing: -0.02em; /* Adjust letter spacing for body */
    }
    ...
    */
    ...
    .interactive-report-body h1, .interactive-report-body h2, .interactive-report-body h3 {
        color: #073B4C; /* Dark Blue for headings */
        font-weight: 700; /* Ensure headings are consistently bold */
        letter-spacing: 0.01em; /* Adjust letter spacing for headings */
    }
    /* Tailwind classes will handle specific heading margins and sizes */

    .interactive-report-body h4 { /* Base h4 style */
        ...
        line-height: 1.4;
        letter-spacing: 0.01em; /* Adjust letter spacing for h4 */
    }
...
<div class="interactive-report-body">
...
document.addEventListener('astro:page-load', function () {
    ...
    const caffeineCanvas = document.getElementById('caffeineContentChart');
    if (!caffeineCanvas) return;
    const existingCaffeine = Chart.getChart(caffeineCanvas);
    if (existingCaffeine) existingCaffeine.destroy();
    const caffeineContentCtx = caffeineCanvas.getContext('2d');
    const caffeineData = {
        labels: [
    ...
    const existingSleep = Chart.getChart(sleepCanvas);
    if (existingSleep) existingSleep.destroy();
    const sleepQualityCtx = sleepCanvas.getContext('2d');
    ...
    const neuroCanvas = document.getElementById('neuroProtectionChart');
    const existingNeuro = Chart.getChart(neuroCanvas);
    if (existingNeuro) existingNeuro.destroy();
    const neuroProtectionCtx = neuroCanvas.getContext('2d');
