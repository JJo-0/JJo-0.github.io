...
![Navier–Stokes 방정식의 항과 정칙성 질문을 연결한 설명 도식](/assets/news-diagrams/navier-equation-map.svg)

도식의 `u`는 속도장, `p`는 압력, `ν`는 점성을 나타냅니다. 왼쪽 항들은 속도가 시간과 공간에서 변하는 모습을, 오른쪽 항들은 압력과 점성이 흐름을 바꾸는 모습을 표현합니다. 핵심 질문은 처음에 잘 정의된 흐름이 시간이 지나도 계속 매끄러운지, 아니면 어느 순간 속도 같은 값이 통제되지 않을 수 있는지입니다.
